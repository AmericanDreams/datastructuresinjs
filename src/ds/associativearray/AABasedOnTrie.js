...
export default class Trie {
  ...
  insert(word, value) {
    ...
    const result = this.insertNode(this.root, word, value);
    if (result) return value;
    return undefined;
  }

  insertNode(node, word, value) {
    ...
    if (word.length === 1) {
      ...
      return this.insertNode(child, word.substr(1, word.length), value);
    }
  }
  ...
  find(word) {
    if (!word || word.length === 0) return undefined;
    return this.findNode(this.root, word);
  }
  ...
  findNode(node, word) {
    ...
    if (!child) return undefined; // So it means this word never inserted before
    ...
    if (word.length === 1 && child) {
      ...
      // We should go bottom still
      return this.findNode(child, word.substr(1, word.length));
    }
  }
  ...
  printNodesSorted(node, word) {
    ...
    //First itself
    if (word !== " " && node.isLastLetter) Util.print(node.value);
